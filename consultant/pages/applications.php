...
$sql = "SELECT a.*, u.full_name, u.email, c.name AS country_name, c.iso2, vt.name AS visa_type_name
        ...
        WHERE a.assigned_consultant_id = :consultant_id AND a.deleted_at IS NULL";
$params = ['consultant_id' => $consultant['id']];
...
render_consultant_start('applications', 'My Applications');
?>
<div class="button-group" style="margin-bottom:var(--space-6);flex-wrap:wrap">
    <a href="/consultant/applications/" class="btn btn-sm <?= $statusFilter === '' ? 'btn-primary' : 'btn-outline' ?>">All</a>
    <?php foreach (application_status_stages() as $key => $label): ?>
    <a href="/consultant/applications/?status=<?= e($key) ?>" class="btn btn-sm <?= $statusFilter === $key ? 'btn-primary' : 'btn-outline' ?>"><?= e($label) ?></a>
    <?php endforeach; ?>
</div>
...
<?php endforeach; ?>
<?php else: ?>
<p class="empty-state">No applications assigned to you<?= $statusFilter !== '' ? ' with this status' : '' ?>.</p>
<?php endif; ?>
