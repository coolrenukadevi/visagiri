...
$cities = $pdo->query("SELECT slug, updated_at FROM cities WHERE indexable = 1 ORDER BY name")->fetchAll();
...
echo '  <url><loc>' . htmlspecialchars(SITE_URL . '/cities/', ENT_XML1) . '</loc><changefreq>monthly</changefreq><priority>0.6</priority></url>' . "\n";
...
foreach ($cities as $c) {
    echo '  <url>';
    echo '<loc>' . htmlspecialchars(SITE_URL . '/cities/' . $c['slug'] . '/', ENT_XML1) . '</loc>';
    echo '<lastmod>' . htmlspecialchars(substr($c['updated_at'], 0, 10), ENT_XML1) . '</lastmod>';
    echo '<changefreq>monthly</changefreq>';
