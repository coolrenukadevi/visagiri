...
foreach ($pdo->query('SELECT slug, updated_at FROM countries WHERE indexable = 1 ORDER BY slug') as $c) {
    $url(SITE_URL . '/visa/' . $c['slug'] . '/', '0.7', 'weekly', $c['updated_at']);
}
...
foreach ($catStmt as $row) {
    $url(SITE_URL . '/visa/' . $row['country_slug'] . '/' . $row['category_slug'] . '/', '0.8', 'monthly', $row['last_reviewed_at']);
}
