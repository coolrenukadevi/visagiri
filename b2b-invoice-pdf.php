<?php
/**
 * Partner-facing invoice PDF — tenant-scoped mirror of
 * admin/b2b-invoice-pdf.php. WHERE additionally requires partner_id to
 * match the logged-in partner so a partner can never download another
 * partner's invoice by guessing/incrementing the id.
 */
...
$pdo = b2b_db();
$stmt = $pdo->prepare('SELECT i.*, p.application_ref, p.company_name, p.contact_name, p.contact_email
    FROM b2b_invoices i JOIN b2b_partners p ON p.id = i.partner_id WHERE i.id = ? AND i.partner_id = ?');
$stmt->execute([$invoiceId, partner_id()]);
...
class B2bPartnerInvoicePdf extends FPDF
{
