...
/** Authenticated download of a B2B Travel Partner KYC document (b2b_partner_documents). */
...
$stmt = db()->prepare('SELECT * FROM b2b_partner_documents WHERE id = :id AND deleted_at IS NULL');
$stmt->execute(['id' => $id]);
$document = $stmt->fetch();
