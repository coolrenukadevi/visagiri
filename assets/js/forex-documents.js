document.addEventListener('DOMContentLoaded', function () {
    var grid = document.getElementById('fxDocGrid');
    if (!grid || typeof FX_DOC_REF === 'undefined') { return; }
    ...
    grid.querySelectorAll('.b2b-doc-file-input').forEach(function (input) {
        input.addEventListener('change', function () {
            ...
            var fd = new FormData();
            fd.append('ref', FX_DOC_REF);
            fd.append('token', FX_DOC_TOKEN);
            fd.append('doc_type', docType);
            fd.append('document', input.files[0]);

            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'forex-document-upload.php', true);
            xhr.onload = function () {
                ...
                if (xhr.status >= 200 && xhr.status < 300 && data.success) {
                    ...
                    filenameEl.outerHTML = '<span class="b2b-doc-filename" data-doc-filename><i class="fa-solid fa-file-lines" aria-hidden="true"></i> ' + data.filename.replace(/[<>&]/g, '') + '</span>';
