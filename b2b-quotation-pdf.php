<?php
/**
 * Partner-facing quotation PDF — tenant-scoped mirror of
 * admin/b2b-quotation-pdf.php. Uses partner session auth instead of
 * admin auth, and the WHERE clause additionally requires partner_id to
 * match the logged-in partner (and hides Draft quotations, same as
 * b2b-quotation.php) so a partner can never download another partner's
 * quotation by guessing/incrementing the id.
 */
...
$ref = trim($_GET['ref'] ?? '');
$pdo = b2b_db();
// Only non-colliding partner columns are selected here — a bare `p.*`
// alongside `q.*` would silently overwrite the quotation's own `id`/
// `status`/`country` with the partner's in PDO::FETCH_ASSOC (see the
// comment in admin/b2b-quotation-pdf.php for the concrete case that bit).
$stmt = $pdo->prepare("SELECT q.*, p.application_ref, p.company_name, p.contact_name, p.contact_email
    FROM b2b_quotations q JOIN b2b_partners p ON p.id = q.partner_id
    WHERE q.quotation_ref = ? AND q.partner_id = ? AND q.status != 'Draft'");
...
class B2bPartnerQuotationPdf extends FPDF
{
...
$pdf = new B2bPartnerQuotationPdf();
$pdf->AliasNbPages();
$pdf->AddPage();
